...
$flashType = $_SESSION['admin_cash_advance_flash_type'] ?? null;
$flashMsg = $_SESSION['admin_cash_advance_flash_msg'] ?? null;
unset($_SESSION['admin_cash_advance_flash_type'], $_SESSION['admin_cash_advance_flash_msg']);
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($employeeId <= 0 || $userId <= 0) {
        $_SESSION['admin_cash_advance_flash_type'] = 'danger';
        $_SESSION['admin_cash_advance_flash_msg'] = 'Unable to identify your admin employee profile.';
        header('Location: cash_advance.php');
        exit;
    }
    if ($amountRaw === '' || !is_numeric($amountRaw) || (float) $amountRaw <= 0) {
        $_SESSION['admin_cash_advance_flash_type'] = 'danger';
        $_SESSION['admin_cash_advance_flash_msg'] = 'Please enter a valid amount.';
        header('Location: cash_advance.php');
        exit;
    }

    $amount = number_format((float) $amountRaw, 2, '.', '');
    $saveNotes = $notes !== '' ? $notes : 'Admin request';
    $insert = $pdo->prepare('
        INSERT INTO cash_advances (employee_id, requested_by_user_id, request_source, amount, notes, advance_date, status)
        VALUES (?, ?, "admin", ?, ?, CURDATE(), "pending")
    ');
    $insert->execute([$employeeId, $userId, $amount, $saveNotes]);

    $_SESSION['admin_cash_advance_flash_type'] = 'success';
    $_SESSION['admin_cash_advance_flash_msg'] = 'Cash advance request forwarded to superadmin.';
    header('Location: cash_advance.php');
    exit;
}
...
if ($employeeId > 0) {
    ...
    $records = $list->fetchAll(PDO::FETCH_ASSOC) ?: [];
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <main class="col-12 col-md-9 col-lg-10 py-4">
          ...
          <p class="text-muted">Submit your own request. Superadmin handles approval.</p>
              ...
              <table class="table table-sm align-middle mb-0">
                ...
                <tbody>
                  <?php if (empty($records)): ?>
                    <tr><td colspan="5" class="text-center text-muted py-3">No cash advance records yet.</td></tr>
                  <?php else: ?>
                    <?php foreach ($records as $row): ?>
                      ...
                      <tr>
                        ...
                        <td><?= htmlspecialchars((string) ($row['employee_code'] ?? 'N/A'), ENT_QUOTES, 'UTF-8') ?></td>
