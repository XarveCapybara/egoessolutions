...
if (!eg_csrf_validate(is_string($csrfToken) ? $csrfToken : null)) {
    if ($wantsJson) {
        ...
        pwd_json_exit(false, 'Security token mismatch. Refresh the page and try again.');
    }
    $_SESSION['password_update_status'] = 'error';
    $_SESSION['password_update_message'] = 'Security token mismatch. Please refresh and try again.';
    header('Location: employees.php');
    exit;
}
