...
// Employee navigation list (comma-separated IDs from payroll.php)
$employeeListRaw = trim((string) ($_GET['employees'] ?? ''));
$employeeIdList = [];
if ($employeeListRaw !== '') {
  foreach (explode(',', $employeeListRaw) as $eid) {
    $eid = (int) trim($eid);
    if ($eid > 0) {
      $employeeIdList[] = $eid;
    }
...
$nextEmployeeId = null;
$currentIndex = -1;
$totalEmployees = count($employeeIdList);
if ($totalEmployees > 0) {
  $currentIndex = array_search($employeeId, $employeeIdList, true);
  if ($currentIndex !== false) {
    if ($currentIndex > 0) {
      $prevEmployeeId = $employeeIdList[$currentIndex - 1];
    }
    if ($currentIndex < $totalEmployees - 1) {
      $nextEmployeeId = $employeeIdList[$currentIndex + 1];
    }
  } else {
    $currentIndex = -1;
  }
...
// Build base query params for navigation links
$navBaseParams = [
  'period' => $period,
  'week' => $weekInput,
  'month' => $monthInput,
  'office_id' => $officeFilter,
  'employees' => $employeeListRaw,
];
function eg_payslip_nav_url(int $eid, array $baseParams): string {
  $baseParams['employee_id'] = $eid;
  return 'payslip_print.php?' . http_build_query($baseParams);
}
...
<head>
  ...
  <style>
    ...
    .eg-payslip-nav-btn:hover {
      background: #6f42c1;
      color: #fff;
    }
    .eg-payslip-nav-btn[disabled],
    .eg-payslip-nav-btn--disabled {
      opacity: 0.35;
      pointer-events: none;
      cursor: default;
    ...
    @media print {
      .eg-payslip-nav { display: none !important; }
    }
  </style>
</head>
...
<body class="eg-payslip-body">
  ...
  <nav class="eg-payslip-nav" aria-label="Employee payslip navigation">
    <?php if ($prevEmployeeId !== null): ?>
      <a href="<?= htmlspecialchars(eg_payslip_nav_url($prevEmployeeId, $navBaseParams), ENT_QUOTES, 'UTF-8') ?>"
         class="eg-payslip-nav-btn" title="Previous employee">
        <i class="bi bi-chevron-left"></i> Previous
      </a>
    <?php else: ?>
      <span class="eg-payslip-nav-btn eg-payslip-nav-btn--disabled"><i class="bi bi-chevron-left"></i> Previous</span>
    <?php endif; ?>

    <span class="eg-payslip-nav-info">
      <?= ($currentIndex !== false && $currentIndex >= 0) ? ($currentIndex + 1) : '?' ?> of <?= $totalEmployees ?>
    </span>

    <?php if ($nextEmployeeId !== null): ?>
      <a href="<?= htmlspecialchars(eg_payslip_nav_url($nextEmployeeId, $navBaseParams), ENT_QUOTES, 'UTF-8') ?>"
         class="eg-payslip-nav-btn" title="Next employee">
        Next <i class="bi bi-chevron-right"></i>
      </a>
    <?php else: ?>
      <span class="eg-payslip-nav-btn eg-payslip-nav-btn--disabled">Next <i class="bi bi-chevron-right"></i></span>
    <?php endif; ?>
  </nav>
  <?php endif; ?>
      ...
      <div class="eg-payslip-sheet" id="egPayslipSheet">
          ...
          <div class="eg-payslip-left">
            <div class="eg-payslip-box">
              ...
              <div class="eg-payslip-field">
                ...
                <div id="ps-name" class="eg-payslip-field-value eg-payslip-field-value--name">
                  <?= htmlspecialchars($fullName, ENT_QUOTES, 'UTF-8') ?></div>
              </div>
              <div class="eg-payslip-field">
                <label for="ps-pos">Position:</label>
                <div id="ps-pos" class="eg-payslip-field-value">
                  <?= htmlspecialchars($positionLabel, ENT_QUOTES, 'UTF-8') ?></div>
              </div>
              <div class="eg-payslip-field">
                <label for="ps-code">Employee Code:</label>
                <div id="ps-code" class="eg-payslip-field-value">
                  <?= htmlspecialchars($employeeCode, ENT_QUOTES, 'UTF-8') ?></div>
              </div>
              <div class="eg-payslip-field">
                <label for="ps-period">Pay Period:</label>
                <div id="ps-period" class="eg-payslip-field-value">
                  <?= htmlspecialchars($payPeriodLabel, ENT_QUOTES, 'UTF-8') ?></div>
              </div>
            </div>
              ...
              <table class="eg-payslip-deductions-inner">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="eg-payslip-num">
                      <span class="eg-payslip-ded-other"
                        data-full="<?= htmlspecialchars($fmtMoney($otherDeductions), ENT_QUOTES, 'UTF-8') ?>"><?= $fmtMoney($otherDeductions) ?></span>
                    </td>
                  </tr>
                  <tr class="eg-payslip-bold">
                    <td>Total Deductions</td>
                    <td class="eg-payslip-num"><span id="egPayslipDedTableTotal"
                        class="eg-payslip-ded-table-total"><?= $fmtMoney($totalDeductionsAll) ?></span></td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="eg-payslip-net-box">
              <div class="eg-payslip-net-inner">
                ...
                <div class="eg-payslip-net-deductions-summary">
                  ...
                  <div class="eg-payslip-num"><span id="egPayslipTotalDedAmount"
                      class="eg-payslip-total-ded-amount"><?= $fmtMoney($totalDeductionsAll) ?></span></div>
                </div>
              </div>
              <div class="eg-payslip-net-total">Net Pay: PHP <span id="egPayslipNetAmount"
                  class="eg-payslip-net-amount"><?= $fmtMoney($net) ?></span></div>
            </div>
          </div>
        </div>
        ...
        <footer class="eg-payslip-footer">
          ...
          <div class="eg-payslip-ack">
            ...
            <p>
              I acknowledge that I have received the amount of <strong>PHP <span id="egPayslipAckAmount"
                  class="eg-payslip-ack-amount"><?= $fmtMoney($net) ?></span></strong> stated in this payslip.
            </p>
